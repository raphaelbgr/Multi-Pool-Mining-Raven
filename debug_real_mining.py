...
def run_real_miner(start_nonce=0, max_wait_time=300):
    """Run the real CUDA miner and wait for a valid nonce"""
    try:
        logging.info(f"Running real CUDA miner with start_nonce={start_nonce}")
        ...
        logging.info("🎯 Real CUDA miner is running... waiting for valid nonce...")
...
def main():
    logging.info("🚀 Starting REAL X16R mining test with 2Miners...")
    ...
    # Subscribe
    subscribe = {"id": 1, "method": "mining.subscribe", "params": ["real-mining-test/0.1"]}
    ...
    if not os.path.exists(MINER_EXE):
        ...
        logging.info("Please ensure miner_x16r_proper.exe is in the current directory")
        s.close()
        return

    # Run real CUDA miner to find a valid nonce (up to 5 minutes)
    found_nonce = run_real_miner(0, 300)  # 5 minutes max
