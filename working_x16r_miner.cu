...
// Real X16R hash function (CPU version)
void x16r_hash_cpu(void *output, const void *input)
{
...
int main(int argc, char **argv) {
    ...
    // CPU mining loop (simplified for now)
    printf("Starting CPU mining with real X16R...\n");
    
    bool found = false;
    uint32_t max_nonces = 1000000; // Test 1M nonces
    
    for(uint32_t nonce = start_nonce; nonce < start_nonce + max_nonces; nonce++) {
        for(int p=0; p<active_pools; p++) {
            int pool_offset = p * POOL_DATA_SIZE;
            
            uint32_t hdr_len = *((uint32_t*)&all_data[pool_offset]);
            if(hdr_len != 80) continue;
            
            uint8_t temp[80];
            uint8_t hash_result[32];
            
            memcpy(temp, &all_data[pool_offset + 4], 80);
            ...
            const uint8_t *target = &all_data[pool_offset + 4 + 80];
            
            // Use real X16R hash function
            x16r_hash_cpu(hash_result, temp);
        ...
        if(nonce % 100000 == 0) {
            printf("Tested %u nonces...\n", nonce - start_nonce);
        }
    }
    
    if(!found) {
        printf("No valid X16R nonces found in %u attempts\n", max_nonces);
    }
