#!/usr/bin/env python3
"""
IMPROVED AUTO MINER X16R PROPER - Ravencoin Multi-Pool Miner
Each pool gets its own header file, test hashes against all targets
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('miner_x16r_improved.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class X16RImprovedMiner:
    def __init__(self):
    ...
    def create_pool_headers(self, jobs: List[Optional[Dict]]) -> List[str]:
        """Create separate header files for each pool"""
        header_files = []
        
        for i, (pool, job) in enumerate(zip(self.config['pools'], jobs)):
            if not job:
                header_files.append(None)
                continue
            
            header_file = f"header_pool_{i}.bin"
            try:
                with open(header_file, "wb") as f:
                    ...
                    if header:
                        ...
                        logger.info(f"Created {header_file} for {pool['name']}")
                    else:
                        header_files.append(None)
                        logger.error(f"Failed to build header for {pool['name']}")
                        
            except Exception as e:
                logger.error(f"Error creating {header_file}: {e}")
                header_files.append(None)
    ...
    def run_x16r_mining_cycle(self, nonce_start: int, header_files: List[Optional[str]]) -> List[Optional[int]]:
        """Run X16R mining cycle with multiple header files"""
        results = [None] * len(self.config['pools'])
        ...
        for i, header_file in enumerate(header_files):
            ...
            try:
                # Run X16R miner for this pool
                cmd = [f"./{self.x16r_miner}", str(nonce_start)]
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
                
                if result.returncode == 0:
                    logger.info(f"X16R mining completed for pool {i}")
                    # Parse results (simplified - in real implementation you'd parse the output)
                    # For now, return None (no solution found)
                    results[i] = None
                else:
                    logger.error(f"X16R mining failed for pool {i}: {result.stderr}")
                    
            except Exception as e:
                logger.error(f"Error in X16R mining for pool {i}: {e}")
    ...
    def run_cycle(self):
        ...
        # Create separate header files for each pool
        header_files = self.create_pool_headers(jobs)
        
        # Run X16R mining for each pool
        results = self.run_x16r_mining_cycle(self.nonce_start, header_files)
    ...
    def start_mining(self):
        """Start X16R mining"""
        logger.info("Starting Improved X16R Ravencoin Miner")
        logger.info(f"Started at: {datetime.now()}")
        logger.info("Using separate header files for each pool")
