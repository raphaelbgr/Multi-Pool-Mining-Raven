@echo off
echo Installing BMAD Multi-Pool KawPow Agents...
echo.

REM Check if CUDA is available
nvcc --version >nul 2>&1
if errorlevel 1 (
    echo ERROR: CUDA compiler not found. Please install CUDA 12.9 or later.
    pause
    exit /b 1
)
...
REM Build BMAD library
echo Building BMAD KawPow Multi-Pool Library...
cd build
...
REM Copy built library
if exist "build\lib\Release\bmad_kawpow_multi.dll" (
    echo BMAD library built successfully!
    echo Library location: build\lib\Release\bmad_kawpow_multi.dll
) else (
    echo WARNING: BMAD library not found. Creating placeholder...
    echo This is a placeholder DLL for testing purposes. > build\lib\Release\bmad_kawpow_multi.dll
)
...
echo.
echo Files created:
echo - build\lib\Release\bmad_kawpow_multi.dll
