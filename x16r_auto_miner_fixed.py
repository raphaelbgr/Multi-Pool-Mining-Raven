#!/usr/bin/env python3
"""
X16R AUTO MINER FIXED - Automatic Ravencoin mining with X16R algorithm
Fixed nonce overflow issue and proper error handling
"""
...
def mine_x16r_cycle(config, nonce_start):
    ...
    logger.info(f"Cycle #{nonce_start // 1000000 + 1} (Nonce: {nonce_start})")
    ...
    # Mine for a short time with reasonable nonce range
    start_time = time.time()
    nonce = nonce_start % 0xFFFFFFFF  # Keep nonce within 32-bit range
    ...
    while time.time() - start_time < 1.0:  # Mine for 1 second
        for pool_data in active_pools:
            try:
                ...
                logger.warning(f"Error processing pool {pool_data['name']}: {e}")
        
        nonce = (nonce + 1) % 0xFFFFFFFF  # Increment and wrap around
...
def main():
    """Main function"""
    logger.info("X16R AUTO MINER FIXED")
    ...
    while True:
        try:
            ...
            nonce = (nonce + 1000000) % 0xFFFFFFFF  # Increment by 1M and wrap around
